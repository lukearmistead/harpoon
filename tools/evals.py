...
def revision():
    ...
    template. Unpushed counts against every remote rather than against the
    branch's upstream, because a branch nobody has pushed yet has no upstream
    and once reported null there, which read as "no git here" on a machine
    that had git and a repo and one unpushed commit.
    """
    def git(*args):
        # Every GIT_ variable is dropped, because -C loses to an ambient
        # GIT_DIR and a hook exports one. Run from .githooks/pre-commit, this
        # stamped the hook's repo no matter which directory it was handed.
...
def reconcile(evals, boarded):
    ...
    if orphans:
        print("\nEach needs a verdict in judgment.csv and a row on board.md, "
