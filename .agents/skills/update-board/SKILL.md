---
name: update-board
description: Maintain the board in pipeline.md. Holds the rules for status, ordering, dates and the Todo list, and runs the freshness pass that re-checks whether posted jobs are still open and whether anything has changed at a watched company. Use when a row changes status, when the board has not been swept in a while, when a posting needs re-verifying, or when asked to tidy or clean up the pipeline.
---

# Update the board

`pipeline.md` is the only place status lives. This file holds the rules for
keeping it true. The board section of `pipeline.md` should carry what the candidate
...
- `watch`: nobody is working it and nothing is waiting on the candidate. **This costs a
  row in `me/channels.md` in the same turn**, because that row is the thing
...
- Moving a company off **Swept and rejected** needs a line saying what changed.
  Usually it is `me/criteria.md`, not the company.
...
is a claim that the job exists. Re-fetch each one through `harpoon.boards`, or
...
each one, check the board row's own criterion in `me/channels.md` and then ask
