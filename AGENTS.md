...
standing rules live in `profile/criteria.md` and win when the two disagree.
Nothing enters this file without their go-ahead, drafted in conversation first.

## Layout

```
board.md          status and next action, never stale, and the one page
                  the candidate reads. Opens with the grammar verdicts follow
profile/          who they are, and what they want
  criteria.md     read it before evaluating anything
  experience.md   the fact base: every number, title, date and scope, never
                  invented. Too long to load whole; use the resume's map below
  positioning.md  the picks in force. Read it before writing about them
  voice.md        how they sound and the tells to avoid. Read before any prose
...
scripts/          the checks git runs on commit. Each opens with a header
                  saying what it fails on, check-all.sh runs all plus the
                  tests, and .githooks/pre-commit runs that. export-template.sh
                  is the one door out, and personal paths never reach it
```

## Standing behaviors

- Ask before committing, always. Edit freely, say what is uncommitted, wait.
- `board.md` is the inbox for everything that needs the candidate, not just
...
- A status change updates `board.md` the same turn, dated, unasked, and so
  does a `profile/meetings/` note, which moves status, verdict and next action.
...
- Every generated file in `apply/` ends with a line naming what was read
  to write it.
...
- `profile/experience.md` holds every number, title, date, and scope claim;
...
- Two markers, one job each. `[check]` means verify this before it is used, and
  ...
  must also be a line on `board.md`** carrying that short distinctive key
  ...
  Open questions live in `learn/lessons.md`, not in the fact base.
- `profile/positioning.md` states picks, not facts, and the fact wins on
...
- A lesson worth keeping becomes a rule the turn it is learned: theirs, in
  `profile/criteria.md`; template-general, a PR upstream. Never here, which a
  template update overwrites.
- **At the end of a skill run that taught something about how the skill should
  work, append a dated entry to `learn/lessons.md`** naming the skill and
...
- **A learning is not a rule until they say so.** When a shape reaches three,
  ...
  `board.md`'s Todo under **Decisions, the candidate's**, aimed at the skill
