...
`python3 -m harpoon.sweep probe <company>`, which finds the ATS board and
...
sys.path.insert(0, ".")  # this runs from the repo root, where harpoon/ sits
from harpoon.sweep import decided_names, norm  # noqa: E402
...
def main():
    ...
    print(f"{len(companies)} companies in the export, "
          f"{len(companies) - len(new)} already decided in pipeline.md, "
