#!/usr/bin/env bash
# Turn the markdown drafts in applications/<slug>/ into files an applicant
...
# A company slug renders that application's drafts into applications/<slug>/,
...
if [ -d "applications/$target" ]; then
  sources=("applications/$target/resume.md" "applications/$target/cover-letter.md")
  out="applications/$target"
  ...
  echo "not found: expected a company slug under applications/ or a path to a .md file"
  exit 1
fi
mkdir -p "$out"
written=()
...
# Most directories under applications/ hold research and no drafts yet, so a
