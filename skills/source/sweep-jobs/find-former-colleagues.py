...
candidate's own corpus: source documents in profile/documents/, the fact base, and
profile/meetings/. A connection named in that corpus whose current company is not
the one given is a sourced ex-colleague.

Run from the repo root:

    python3 skills/source/sweep-jobs/find-former-colleagues.py "Acme Health"

The report prints to stdout. Review it by hand, then write the survivors to
profile/network/roster-<slug>.md with a line per person naming the source that
...
CORPUS_GLOBS = [
    ...
    "profile/documents/**/*.ipynb",
    "profile/documents/**/*.md",
    "profile/documents/**/*.docx",
    "profile/documents/**/*.xlsx",
    "profile/documents/**/*.pptx",
    "profile/meetings/*.md",
...
def main():
    ...
    corpus = read_corpus()
    if not corpus:
        print("no corpus: drop source documents in profile/documents/ first")
        sys.exit(1)
