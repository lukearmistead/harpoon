...
**1. Intake.** Read every document in `profile/documents/`. Quote numbers verbatim
...
`profile/network/export/Positions.csv` role by role and ask what happened that
...
- **The fact goes in `profile/experience.md`**, which holds every number, title,
...
- **What is still open goes in `learn/lessons.md`** under `# Open questions
  about the record`, with an `[ask: <Key>]` and a matching Todo line if only
  the candidate can answer it.

Voice is none of the three: how the candidate sounds lives in `profile/voice.md`.
...
- A pick is theirs: which identity leads, whether an open question is
  settled. Ask with a recommendation, record it in `profile/positioning.md`,
  then let everything downstream follow.

## Finish

Read the open questions list back: what got filled, what is still open. Then
name what runs next. An empty `profile/criteria.md` means write-criteria; a stale
`profile/resume.md` means write-resume.
...
settled on the board and in `profile/criteria.md`, because nothing carried the
