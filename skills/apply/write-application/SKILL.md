...
One seat at a time, ending in a directory at `apply/<slug>/`. Runs
...
- `profile/criteria.md`, for the thesis and how the candidate wants to be
  worked with.
- `apply/<slug>/company.md`: the verdict, the open questions, who is
  on the other side of the model. Missing? Stop and run research-company.
- `profile/positioning.md`, the picks in force. Empty? Stop and run
  gather-experience.
- `profile/voice.md` before any prose. It holds both halves: how the candidate
  sounds, and the tells that give an LLM away.
- `profile/resume.md`, the master. The variant is cut from it, never rewritten.
  **Its `## What backs each bullet` map is how you reach the fact base.** Read
  the bullet first; open the `profile/experience.md` section it names only when this
  seat needs more than the bullet says, which a letter usually does for one or
  two projects and a resume variant usually does for none. `profile/experience.md`
...
`.claude/skills/source/sweep-jobs/ats-boards.md`) and capture the exact title, location,
...
Three stops: no verdict in `apply/<slug>/company.md` (research
...
Open `profile/positioning.md` and each project's inline picks
in `profile/experience.md` and read what they say today: a cached list goes
...
`profile/experience.md`: a number not in there does not go in. Bullets matching
...
`profile/positioning.md`: a variant is shorter, not longer. The summary line
comes from `profile/resume.md` as it stands; a different emphasis is a
positioning decision, so ask, and record it in `profile/experience.md` first.
...
`apply/<slug>/form-fill.md`, plus anything the form said that the
...
number, title, date and scope claim checked back to `profile/experience.md` and
