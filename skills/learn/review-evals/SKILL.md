---
name: review-evals
description: Review the eval log and act on what it shows. Grades the decisions recorded in learn/runs/<run-id>/, counts shapes toward the third that amends a skill, and puts the fix in the code or the rules where it belongs. Use when asked to review the evals, improve how a step judges, check whether a gate is working, or when a call has proven wrong and needs recording.
---
...
`profile/criteria.md`, before the first CSV. It is the standard every judgment call
...
`learn/lessons.md`, all of it, because counting what repeats is this skill's job and
...
`learn/lessons.md` entries and adjudicated shapes are counted the same way, and
neither promotes itself. When a shape reaches three, put **one line** on
`board.md`'s Todo under **Decisions, the candidate's**: the shape, how many
...
- `profile/criteria.md` is the candidate's. Findings about it are a proposed diff and
  a line on `board.md`, never an edit.
