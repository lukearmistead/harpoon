"""Index the directories an agent would otherwise have to open file by file.
    ...
    python3 -m harpoon.notes            rewrite every index
    python3 -m harpoon.notes --check    fail if one is stale or unsummarized

Four directories, two shapes. `me/meetings/` and `me/interviews/` hold dated
notes, one file per conversation, and their index is one line per note, newest
first. `me/experience/` and `me/network/` hold imported material, 257 files of
...
index reads the same on every clone. `me/network/export/` is gitignored and
...
DATED_DIRS = ("me/meetings", "me/interviews")
MATERIAL_DIRS = ("me/experience", "me/network")
...
def material(directory):
    ...
    for `me/experience/`, several of them five levels down and holding nine
...
def check(directory, kind="dated"):
    ...
    if not index.exists() or index.read_text() != render(directory, kind):
        problems.append(f"{index}: stale, run `python3 -m harpoon.notes`")
